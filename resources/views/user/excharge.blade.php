...
@section('content')
    <div>
        @php
            $chargeinfo = Session::get('form-excharge');
        @endphp
        @if (\Session::has('result'))
            @if (\Session::get('result') == 'success')
                <div id="flashMessage" class="message success" style=''>관리자 환전성공!</div>
            @elseif (\Session::get('result') == 'pass')
                <div id="flashMessage" class="message error" style=''>관리자 비번틀림!</div>
            @else
                <div id="flashMessage" class="message error" style=''>관리자 환전실패!</div>
            @endif
        @endif
    </div>
            ...
            <form action="" novalidate="novalidate" autocomplete="off" id="UserAdminChargeForm" method="post"
                ...
                <div class="row mt-3">
                    <div class="col-12">
                        ...
                        <div class="submit">
                            <input type="submit" value="환전하기" class="btn btn-secondary btn-sm">
